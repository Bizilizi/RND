...
for task_id in range(6):
    ...
    out_path = (
        "/hpc/group/carin/mz149/code/"
        + "/Six_butterfly_task%d_lowrank_highLayers/" % task_id
    )
    ...
    for epoch_idx in range(Num_epoch):
        ...
        for x_real, y in train_loader:
            ...
            gloss, x_fake, l1_loss = trainer.generator_trainstep(
                y, z, Iterr=it, is_lowrank=True
            )
            ...
            with torch.no_grad():
                ...
                if (it % config["training"]["sample_every"]) == 0:
                    ...
                    print(
                        "[epoch %0d, it %4d] g_loss = %.4f/%.4f, d_loss = %.4f, reg=%.4f, d_fix=%.4f, d_update=%.4f, g_fix=%.4f, g_update=%.4f, time=%.2f"
                        % (
                            epoch_idx,
                            it,
                            gloss,
                ...
                if ((it + 1) % 5000) == 0:
                    ...
                    (
                        ...
                        inception_std,
                        fid,
                    ) = evaluator.compute_inception_score(
                        is_FID=True, exit_real_ms=exit_real_ms
                    )
                    ...
                    fid_all.append(fid)
                    print(
                        "test it %d: IS: mean %.2f, std %.2f, FID: mean %.2f, time: %2f"
                ...
                if it + 1 == 20000:
                    ...
                    torch.save(
                        discriminator_part,
                        save_dir + TrainModeSave + "Pre_discriminator",
                    )
