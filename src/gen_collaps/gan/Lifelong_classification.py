...
if do_method == "MeRGAN":
    ...
    for task_id in range(6):
        ...
        dict_G = torch.load(
            model_file + task_name[task_id] + "_%08d_Pre_generator" % 59999
        )
        generator = model_equal_part_embed(generator, dict_G)
...
for task_id in range(0, 6):
    ...
    if do_method == "Joint":
        ...
        train_dataset = datasets.ImageFolder(
            os.path.join(train_path), data_transforms["train"]
        )
        train_loader = torch.utils.data.DataLoader(
            train_dataset, batch_size=n_c, shuffle=True, num_workers=0
        )
    else:
        n_c = batch_size
        n_p = batch_size
    ...
    if do_method == "Joint":
                ...
                if IT % 50 == 0:
                    with torch.no_grad():
                        ...
                        for i_t in range(task_id + 1):
                            ...
                            test_dataset_i = datasets.ImageFolder(
                                os.path.join(test_path_i), data_transforms["test"]
                            )
                            ...
                            test_acc_i = evalu_my(
                                classify_my, test_loader_i, test_task=i_t
                            )
                    ...
                    with torch.no_grad():
                        ...
                        for i_t in range(task_id):
                            ...
                            train_dataset_0 = datasets.ImageFolder(
                                os.path.join(train_path_0), data_transforms["train"]
                            )
                ...
                if task_id > 0:
                    with torch.no_grad():
                        if do_method == "MeRGAN":
                            ...
                            for i_t in range(task_id):
                                y_replay0 = y_0 + i_t * 6
                                x_replay0, _ = generator(
                                    z, y_replay0 + 1, task_id=task_id
                                )
                                x_replay0 = F.interpolate(
                                    x_replay0, 224, mode="bilinear"
                                )
                                mu_0 = (
        ...
        acc_task = []
        acc_task_i = []
        train_loader_fisher = torch.utils.data.DataLoader(
...
sio.savemat(
    save_path + do_method + "_insect9_res18_test_acc_layerAll_lambda_OPT_ALL2.mat",
    {
        "ACC_all_0": ACC_all_0,
        "ACC_all_1": ACC_all_1,
        "ACC_all_2": ACC_all_2,
