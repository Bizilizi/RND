...
def load_part_model(m_fix, m_ini):
    ...
    dict_fix = {
        ...
        if k in dict_ini and k.find("embedding") == -1 and k.find("fc") == -1
    }
...
def model_equal_part(model, dict_all):
    ...
    dict_fix = {
        ...
        if k in model_dict and k.find("embedding") == -1 and k.find("fc") == -1
    }
...
def model_equal_part_embed(model, dict_all):
    ...
    dict_fix = {
        k: v
        for k, v in dict_all.items()
        if k in model_dict and k.find("embedding") == -1
    }
    model_dict.update(dict_fix)
    model.load_state_dict(model_dict)
    return model
...
def out_bias_to_in_bias(model, dict_all):
    ...
    dict_fix = {
        ...
        for k, v in dict_all.items()
        if k in model_dict
        and k.find("AdaFM_fc_b") == -1
        and k.find("AdaFM_b0") == -1
        and k.find("AdaFM_b1") == -1
    }
    for k, v in dict_all.items():
...
def model_equal_SVD(model, dict_all, FRAC=0.9):
    ...
    for k, v in dict_all.items():
        if k.find("AdaFM") >= 0 and k.find("style_gama") >= 0:
            ...
            v_new = 1.0 + (Ua[:, :NUM] * Sa[:NUM].unsqueeze(0)).mm(Va[:, :NUM].t())
...
def model_equal_SVD_v2(model, dict_all, task_id=-1, NUM=200, dim_z=-1):
    ...
    dict_fix = {
        k: v
        for k, v in dict_all.items()
        if k in model_dict and k.find("AdaFM_0") == -1 and k.find("AdaFM_1") == -1
    }
    if dim_z >= 0:
        for k, v in dict_all.items():
            if k.find("fc") >= 0 and k.find("weight") >= 0:
        ...
        if ind >= 0 and k.find("style_gama") >= 0:
            ...
            if task_id >= 1:
                if S_rep == "abs":
                    ...
                    model_dict[k[: ind + 7] + ".gamma_s2"] = (Sa[:NUM] * pecen).pow(
                        1.0 / genh
                    )
                elif S_rep == "exp":
            ...
            if task_id >= 1:
                if S_rep == "abs":
                    ...
                    model_dict[k[: ind + 7] + ".beta_s2"] = (Sa[:NUM] * pecen).pow(
                        1.0 / genh
                    )
                elif S_rep == "exp":
...
def model_equal_SVD_fenkai(model, dict_all, NUM1=100, NUM2=50):
    ...
    for k, v in dict_all.items():
        if k.find("AdaFM") >= 0 and k.find("style_gama") >= 0:
            ...
            v_new = torch.mm(
                torch.mm(Ua[:, :NUM1], torch.diag(Sa[:NUM1])), Va[:, :NUM1].t()
            )
            dict_all[k] = v_new.unsqueeze(2).unsqueeze(3)
        if k.find("AdaFM") >= 0 and k.find("style_beta") >= 0:
            ...
            v_new = torch.mm(
                torch.mm(Ua[:, :NUM2], torch.diag(Sa[:NUM2])), Va[:, :NUM2].t()
            )
            dict_all[k] = v_new.unsqueeze(2).unsqueeze(3)
...
transform = transforms.Compose(
    [
        ...
        transforms.Lambda(lambda x: x + 1.0 / 128 * torch.rand(x.size())),
    ]
)
...
def get_parameter_num(model, task_id=0):
    ...
    p_num += (
        model.AdaFM_fc.gamma.shape[1]
        + model.AdaFM_fc.beta.shape[1]
        + model.AdaFM_fc.b.shape[0]
    )
...
def load_model_norm_svd_S100(model, is_G=True, is_first_task=True):
    ...
    for k, v in dict_all.items():
        ...
        if idt >= 0:
            ...
            dict_all[k[:idt] + "AdaFM_0.gamma_v"].data = Va[:, jj[:real_rank]].data
            if is_first_task:
                dict_all[k[:idt] + "AdaFM_0.gamma_s2"].data = (
                    Sa[jj[:real_rank]].data / S_scale_g
                )
            ...
            dict_all[k[:idt] + "AdaFM_0.beta_v"].data = Va[:, jj[:real_rank]].data
            if is_first_task:
                dict_all[k[:idt] + "AdaFM_0.beta_s2"].data = (
                    Sa[jj[:real_rank]].data / S_scale_b
                )
        idt = k.find("conv_1.weight")
        if idt >= 0:
            w_mu = v.mean([2, 3], keepdim=True)
            ...
            dict_all[k[:idt] + "AdaFM_1.gamma_v"].data = Va[:, jj[:real_rank]].data
            if is_first_task:
                dict_all[k[:idt] + "AdaFM_1.gamma_s2"].data = (
                    Sa[jj[:real_rank]].data / S_scale_g
                )
            ...
            dict_all[k[:idt] + "AdaFM_1.beta_v"].data = Va[:, jj[:real_rank]].data
            if is_first_task:
                dict_all[k[:idt] + "AdaFM_1.beta_s2"].data = (
                    Sa[jj[:real_rank]].data / S_scale_b
                )
...
def load_model_norm_svd_AR(model, dict_all, is_G=True, is_first_task=True):
    ...
    dic_choose = {
        k: v
        for k, v in dict_all.items()
        if k in model_dict and (k.find("AdaFM_0") == -1 or k.find("AdaFM_1") == -1)
    }
    for k, v in dict_all.items():
        if k in model_dict:
