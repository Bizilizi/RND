...
class ResnetBlock_style_multiTask(nn.Module):
    ...
    def forward(self, x, task_id=-1, UPDATE_GLOBAL=False, Iterr=1, device=None):
        if not UPDATE_GLOBAL:
            ...
            self.AdaFM_0.update_global(
                self.fhidden, self.fin, task_id=task_id, UPDATE_GLOBAL=UPDATE_GLOBAL
            )
            self.AdaFM_1.update_global(
                self.fout, self.fhidden, task_id=task_id, UPDATE_GLOBAL=UPDATE_GLOBAL
            )
...
class AdaFM_SVD(nn.Module):
    def __init__(
        ...
        self.style_gama = nn.Parameter(
            stdd2 + stdd2 * torch.randn(in_channel, out_channel, 1, 1)
        )
        self.style_beta = nn.Parameter(
            stdd2 + stdd2 * torch.randn(in_channel, out_channel, 1, 1)
        )
        if is_use_bias:
            self.b = nn.Parameter(torch.zeros(in_channel))
    ...
    def forward(self, input=0, W=0, b=0, b_i=0.0, task_id=-1, Iterr=1):
        if task_id >= 0:  # test history tasks
            ...
            uu = list_2_matrix(
                self.global_gamma_u[: task_id + 1], global_num=self.global_num_gamma
            )
            vv = list_2_matrix(
                self.global_gamma_v[: task_id + 1], global_num=self.global_num_gamma
            )
            gamma = (uu * ss.unsqueeze(0)).mm(vv.t())

            ss = self.global_beta_s[task_id]
            uu = list_2_matrix(
                self.global_beta_u[: task_id + 1], global_num=self.global_num_beta
            )
            vv = list_2_matrix(
                self.global_beta_v[: task_id + 1], global_num=self.global_num_beta
            )
            ...
            if self.global_num_task > 0:
                ss = self.gamma_s1
                uu = list_2_matrix(
                    self.global_gamma_u, global_num=self.global_num_gamma
                )
                vv = list_2_matrix(
                    self.global_gamma_v, global_num=self.global_num_gamma
                )
                gamma = (uu * ss.unsqueeze(0)).mm(vv.t())
                ss = self.beta_s1
        ...
        W_i = gamma.to(input.device).unsqueeze(2).unsqueeze(3) * W.to(
            input.device
        ) + beta.to(input.device).unsqueeze(2).unsqueeze(3)
        out = F_conv(input, W_i, bias=b + b_i, stride=1, padding=1)
        return out
    ...
    def update_global(
        ...
        if self.global_num_task >= 0:
            with torch.no_grad():
                ...
                if s_sum / ii_acsum[-1] < FRAC:
                    ...
                    if self.global_num_task == 0:
                        ...
                        s_all = torch.cat(
                            (my_copy(self.gamma_s1), my_copy(Sa[jj[:num]])), 0
                        )
                ...
                if s_sum / ii_acsum[-1] < FRAC:
                    ...
                    if self.global_num_task == 0:
                        ...
                        s_all = torch.cat(
                            (my_copy(self.beta_s1), my_copy(Sa[jj[:num]])), 0
                        )
            ...
            self.gamma_s1 = nn.Parameter(
                stdd4 * torch.randn(sum(self.global_num_gamma), device=device)
            )
            # self.gamma_s1.data[:self.global_num_gamma[0]] = self.global_gamma_s[0].data
            self.gamma_s1.data[: sum(self.global_num_gamma)] = self.global_gamma_s[
                -1
            ].data
            ...
            self.beta_s1 = nn.Parameter(
                stdd4 * torch.randn(sum(self.global_num_beta), device=device)
            )
...
class ResnetBlock_style_SVD(nn.Module):
    ...
    def forward(
        ...
        if not UPDATE_GLOBAL:
            ...
            dx = actvn(x)
            dx = self.AdaFM_0(
                dx, self.conv_0.weight, self.conv_0.bias, task_id=task_id, Iterr=Iterr
            )

            dx = actvn(dx)
            dx = self.AdaFM_1(
                dx, self.conv_1.weight, self.conv_1.bias, task_id=task_id, Iterr=Iterr
            )
