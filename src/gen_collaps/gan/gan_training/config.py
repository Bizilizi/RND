...
def build_optimizers(generator, discriminator, config):
    ...
    if equalize_lr:
        ...
        g_params = get_parameter_groups(
            generator.parameters(), g_gradient_scales, base_lr=lr_g
        )
        d_params = get_parameter_groups(
            discriminator.parameters(), d_gradient_scales, base_lr=lr_d
        )
    else:
    ...
    if optimizer == "rmsprop":
        ...
        g_optimizer = optim.Adam(g_params, lr=lr_g, betas=(0.0, 0.99), eps=1e-8)
        d_optimizer = optim.Adam(d_params, lr=lr_d, betas=(0.0, 0.99), eps=1e-8)

    elif optimizer == "sgd":
        g_optimizer = optim.SGD(g_params, lr=lr_g, momentum=0.0)
        d_optimizer = optim.SGD(d_params, lr=lr_d, momentum=0.0)
...
def build_optimizers_E(encoder, config):
    ...
    if equalize_lr:
        ...
        e_params = get_parameter_groups(
            encoder.parameters(), e_gradient_scales, base_lr=lr_e
        )
    else:
    ...
    if optimizer == "rmsprop":
        ...
        e_optimizer = optim.SGD(e_params, lr=lr_e, momentum=0.0)
