...
class Evaluator(object):
    ...
    def compute_inception_score(
        self, task_id=-1, is_FID=True, exit_real_ms=False, real_m=0, real_s=0
    ):
        self.generator.eval()
        imgs = []
        ...
        if exit_real_ms:
            fid = calculate_fid_given_real_ms(
                real_m, real_s, fid_imgs, batch_size=self.batch_size, cuda=True
            )
        else:
    ...
    def compute_inception_score_MeRGAN(
        self, task_id=-1, is_FID=True, exit_real_ms=False, real_m=0, real_s=0
    ):
        self.generator.eval()
        imgs = []
    ...
    def create_samples(self, z, y=None):
        ...
        if y is None:
            ...
            y = torch.full((batch_size,), y, device=self.device, dtype=torch.int64)
        # Sample x
        with torch.no_grad():
