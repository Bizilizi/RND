...
class VisualizeProjections(Callback):
            ...
            for ratio in [0.75, 0.4, 0.1, 0]:
                if isinstance(logger, WandbLogger):
                    ...
                    real_image_embs, real_classes = self.project_dataset(
                        model, dataset_to_project, ratio=ratio
                    )
                    ...
                    wandb.log(
                        {
                            f"train/projections/{ratio}/real_data_experience_step_{experience_step}": wandb.Table(
                    ...
                    if experience_step > 0:
                        ...
                        ) = self.project_dataset(model, dataset_to_project, ratio=ratio)
                        ...
                        wandb.log(
                            {
                                f"train/projections/{ratio}/bootstrapped_data_experience_step_{experience_step}": wandb.Table(
                                    columns=["x", "y", "class", "is_real"],
                    ...
                    wandb.log(
                        {
                            f"train/projections/{ratio}/all_data_experience_step_{experience_step}": wandb.Table(
    ...
    def project_dataset(self, model, dataset_to_project, ratio):
        experience_step = model.experience_step
        ...
        for x, y, _ in dataloader:
            ...
            x = x.to(model.device)
            full_features, _ = model.encoder(x, ratio=ratio)
