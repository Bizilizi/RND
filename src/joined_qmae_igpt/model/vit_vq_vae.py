...
from src.joined_qmae_igpt.model.image_gpt import ImageGPTForCausalImageModeling
from src.joined_qmae_igpt.model.decoder import MAEDecoder
from src.joined_qmae_igpt.model.encoder import MAEEncoder
from src.joined_qmae_igpt.model.quiantizer import (
    VectorQuantizerEMA,
)
from src.joined_qmae_igpt.train_image_gpt import sample_images, get_image_embedding

if t.TYPE_CHECKING:
    from src.joined_qmae_igpt.model.classification_head import EmbClassifier
...
class VitVQVae(CLModel):
    def __init__(
        ...
        """
        mask_token = self.feature_quantization.num_embeddings
        sos_token = self.feature_quantization.num_embeddings + 1

        """
        vocab_size = num_embeddings + mask_token + igpt_sos_token + num_classes
        """
        vocab_size = self.feature_quantization.num_embeddings + 2 + 10
    ...
    def criterion(self, forward_output: ForwardOutput, y) -> CriterionOutput:
        ...
        # )
        igpt_input = self._extend_with_class_token(x_indices, y)
        igpt_input = self._extend_with_sos_token(x_indices)

        igpt_output = self.image_gpt(input_ids=igpt_input)
        igpt_loss = F.cross_entropy(
            igpt_output.logits[:, :-1].reshape(-1, igpt_output.logits.shape[-1]),
            x_indices[..., 1:].reshape(-1),
        )
    ...
    def on_train_epoch_end(self):
        ...
        if random.uniform(0, 1) > 0.5:
            image_embeddings = get_image_embedding(
                self, self.config, self.mask_token
            ).to(self.device)

            sample = sample_images(
                image_gpt=self.image_gpt,
                vq_vae_model=self,
                embedding=image_embeddings,
                sos_token=self.sos_token,
                return_grid_only=True,
                temperature=1.23,
                classes_to_sample=range(self.experience_step),
            ).cpu()
